#### problem with rtracklayer usage for disaplying tracks via UCSC:
# see https://github.com/lawremi/rtracklayer/issues/113
...
## I was getting a different error that's probably related, when I was trying to use `browserView()` for the SATAY project. See ~/FH_fast_storage/forOtherPeople/forGrantKing/SATAY/janet_Rscripts/writeBedForUCSC.Rmd

# xxx Error in h(simpleError(msg, call)) : 
# error in evaluating the argument 'object' in selecting a method for function 'ucscTrackModes': attempt to set an attribute on NULL


##  I tried with rtracklayer_1.62.0


### R code from vignette source 'rtracklayer.Rnw'
# https://www.bioconductor.org/packages/release/bioc/vignettes/rtracklayer/inst/doc/rtracklayer.R
