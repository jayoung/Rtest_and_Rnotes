...
```
If there's an empty group, spacing gets weird, so we do `geom_boxplot(position = position_dodge(preserve = "single"))`.
...
The `position_dodge2` function gives ALMOST the same output, but the alignment of the 'versicolor' box is different:
