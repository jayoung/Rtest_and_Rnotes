...
#### combine_myGGmarginal_plots is for when I want to show several plots generated by my_ggMarginal together, side-by-side
combine_myGGmarginal_plots <- function(list_of_plot_lists, 
                                                 relative_widths=c(6,1), 
                                                 quiet=TRUE) {
    
    ## rearrange the plot into a list
    rearranged_plot_list <- list()
    if(!quiet) {
        cat("rearranged_plot_list length = ", length(rearranged_plot_list), "\n")
        cat("  top row\n")
    }
    ## first the top row
    for(i in 1:length(list_of_plot_lists)) {
        if(!quiet) {cat("    top row i=",i, "\n")}
        rearranged_plot_list <- c(rearranged_plot_list, 
                                  list_of_plot_lists[[i]]["dens_x"],
                                  list(plot_spacer()) )
        if(!quiet) {cat("    rearranged_plot_list length = ", length(rearranged_plot_list), "\n")}
    }
    if(!quiet) {cat("  bottom row\n") }
    ## now the bottom row
    for(i in 1:length(list_of_plot_lists)) {
        if(!quiet) {cat("    bottom row i=",i, "\n") }
        rearranged_plot_list <- c(rearranged_plot_list, 
                                  list_of_plot_lists[[i]]["main_plot"],
                                  list_of_plot_lists[[i]]["dens_y"] )
        if(!quiet) {cat("    rearranged_plot_list length = ", length(rearranged_plot_list), "\n") }
    }
    # return(rearranged_plot_list)
    ## now actually make the plot
    wrap_plots(rearranged_plot_list,
               ncol = 2*length(list_of_plot_lists),
               nrow = 2, byrow=TRUE,
               widths = rep(relative_widths, times=length(list_of_plot_lists)), 
               heights = rev(relative_widths), 
               guides="collect")
