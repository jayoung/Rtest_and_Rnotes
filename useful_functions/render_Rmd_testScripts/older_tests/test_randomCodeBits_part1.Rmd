...
This is the first in a set of two linked Rmd scripts, to help me test my `render_Rmd_series.pl` script.

The render_Rmd_series.pl script is set up so that it will not run downstream scripts if anything in the chain fails.
...
Here's some code that will produce an error, if we were to change `eval` to `TRUE.` The render_Rmd_series.pl script is set up so that it will not run downstream scripts if anything in the chain fails.
...
save(iris_plus_groups, 
     file=here("useful_functions/render_Rmd_testScripts/older_tests/iris_plus_groups.Rdata"))
```
